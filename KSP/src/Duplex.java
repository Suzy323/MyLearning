...
public class Duplex {
	...
	public double dijkstra(int[] paths,double[] dist, int start, int end){
		...
		for(i=0;i<g_N;i++)
		{
			paths[i]=-1 ;	
			used[i] = 0;
			dist[i] = (array[start][i]< 0.0) ? POSITIVE_INFINITY : array[start][i]; 
		}	
		...
		{		
			...
			for(i=0;i<g_N;i++)
			{
				...
				double w = array[min_node][i] < 0.0 ? POSITIVE_INFINITY : array[min_node][i];
	...
	public double[][] GraphInitial (double[][] a){
		array = a;
		g_N=a.length;
		g_size=a.length;
		return array;
...
//				System.out.println("path = "+path);
				...
				{
					if( array[i][node] >= 0 && i != nextnode) count++;
					if( i != preNode) 
					{
						adjoinNode = i;
